<?php
session_start();
class quote_general_new extends Model {
    ...
    public function quotePlanbenefis($idCategoria, $diasxpersona, $id_country_broker, $destino, $origen, $edades, $desde, $hasta, $id_broker='', $id_plan_cotiza=false, $id_multiple_plans=false, $ignoreRestrictions=false, $isGroup=false, $intervalo='',$prefix) {
        $arrPlan          = $this->Get_Plans_categoria($idCategoria, '', '', $id_plan_cotiza, $id_multiple_plans, $id_broker, $intervalo,$prefix);
    			...
    			if($arrPlan[$i]['impuesto'] == '1'){    
    				...
    				if(count($arrImpuesto) > 0){    						
    					if($arrImpuesto[0]['tax1'] > 0 &&  $arrImpuesto[0]['status1'] == 1){
    						...
    						$tax1 = $this->truncateFloat(($arrImpuesto[0]['tax1'] * ($total_neto / 100)),3);
    						$actTax = 1;
    					}
    					if($arrImpuesto[0]['tax2'] > 0 &&  $arrImpuesto[0]['status2'] == 1){
    						$porcTax2 = $arrImpuesto[0]['tax2'];
    						$tax2 = $this->truncateFloat(($arrImpuesto[0]['tax2'] * ($total_neto / 100)),3);
    						$actTax = 1;
    					}
    				}
	...
	function Get_Plans_categoria($id_plan_categoria = '', $max_tiempo = 0, $origen="", $id_plan="", $id_multiple_plans="", $id_broker = '', $opcion_plan = '',$prefix) {
		...
		$query = "SELECT
                ...
                FROM
                plans
                LEFT JOIN restriction ON plans.id = restriction.id_plans AND restriction.prefijo = '$prefix'
                LEFT JOIN broker ON broker.id_broker = restriction.id_broker AND broker.prefijo='$prefix'
                LEFT JOIN currency ON plans.id_currence = currency.id_currency
                INNER JOIN plan_detail ON plans.id = plan_detail.plan_id AND plan_detail.prefijo='$prefix'
                WHERE
                plans.activo = '1'
                AND plans.modo_plan = 'W'
                AND eliminado = '1' AND plans.prefijo='$prefix' ";
            if(!empty($id_broker)){
